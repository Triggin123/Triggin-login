...
import { getSellers, searchSeller, saveSeller, SaveSeller, GetSellers } from "../actions/sellers"
...
const Sellers = (props) => {
  ...
  const search_user = useSelector(state => state?.seller?.search);
  ...
  const columns = [
    {
      title: "Name",
      cell: (row) => {
        return <span>{row?.sellerId?.name}{" "}</span>
      }
    },
    {
      title: "Email",
  ...
  return (
    <div className='wrapper'>
      ...
      <TableHead addText={"Sellers"} setOpen={setOpen} count={records ? records.length : 0} totalCount={totalRecords ? totalRecords : 0} />
